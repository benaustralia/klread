...
type ClassNote = {
  ...
  lineId: string; lineIdTo?: string; charStart?: number; charEnd?: number
  body: string; updatedAt: string
}

export function AllNotesSheet({ studentId, joinCode, open, onOpenChange, isTeacher }: {
  studentId: string; joinCode: string; open: boolean; onOpenChange: (v: boolean) => void; isTeacher?: boolean
}) {
  ...
  function exportNotes() {
    ...
    a.download = 'king-lear-notes.txt'
    a.click()
  }
      ...
      <SheetContent side="right" className="flex flex-col w-full sm:max-w-xl" aria-describedby={undefined}>
        ...
        <div className="overflow-y-auto flex-1 px-4 py-2 flex flex-col gap-2">
          ...
          {notes.map(n => {
            ...
            const isEditing = n.id in editing
            const lineRef = n.lineIdTo && n.lineIdTo !== n.lineId ? `${n.lineId}–${n.lineIdTo}` : n.lineId
            return (
              <div key={n.id} className={`border-2 rounded-base p-3 ${mine ? 'border-foreground' : 'border-border bg-secondary-background'}`}>
                <div className="flex items-center justify-between mb-1 gap-2">
                  ...
                  <button
                    ...
                    onClick={() => window.open(`/?goto=${n.lineId}`, '_blank')}
                  >{lineRef}</button>
                </div>
                {isEditing ? (
                  <div className="grid gap-2 mt-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="neutral" onClick={() => setEditing(p => { const c = { ...p }; delete c[n.id]; return c })}>Cancel</Button>
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    {mine && (
                      <div className="flex gap-3 mt-2">
                        <button className="text-xs text-muted-foreground hover:text-foreground" onClick={() => setEditing(p => ({ ...p, [n.id]: n.body }))}>Edit</button>
                        <button className="text-xs text-muted-foreground hover:text-destructive" onClick={() => del(n.id)}>Delete</button>
                      </div>
                    )}
                  </>
                )}
        ...
        <div className="px-4 pb-4 flex gap-2">
          <Button variant="neutral" size="sm" onClick={exportNotes} disabled={!notes.length}>Export</Button>
