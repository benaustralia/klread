...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const rows = name
      ? await sql`UPDATE sessions SET last_seen_at = now() WHERE join_code = ${code} AND student_name = ${name} RETURNING student_id AS "studentId", student_name AS "studentName", join_code AS "joinCode", initials, bookmark_line_id AS "bookmarkLineId", (SELECT is_teacher FROM classes WHERE join_code = ${code}) AS "isTeacher"`
      : await sql`SELECT s.student_id AS "studentId", s.student_name AS "studentName", s.join_code AS "joinCode", s.initials, s.bookmark_line_id AS "bookmarkLineId", c.is_teacher AS "isTeacher" FROM sessions s JOIN classes c ON c.join_code = s.join_code WHERE s.join_code = ${code} LIMIT 1`
